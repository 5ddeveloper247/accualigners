...
@section('content')
    @php
        ...
        $viewRoute = route('admin.patient.index');

        $edit_id = false;
        $form_action = route('admin.patient.store');

        if(Request()->route('patient')){
            $edit_id = Request()->route('patient');
            $form_action = route('admin.patient.update', $edit_id);
        }
    @endphp
                    ...
                    <div class="row breadcrumbs-top">
                        <div class="breadcrumb-wrapper col-12">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item"><a href="{{$viewRoute}}">Patients</a></li>
                                <li class="breadcrumb-item active">{{ $edit_id ? 'Edit' : 'Add New' }} Patient</li>
                            </ol>
                        </div>
                    </div>
                </div>
                                        ...
                                        <div class="card-content collapse show">
                                            <div class="card-body card-dashboard">
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="form-group col-xl-6 col-lg-6 col-md-6 col-sm-12 <?php if ($errors->has('clinic_id')) echo 'error'; ?>">
                                                        <label>Clinic <span class="required">*</span></label>
                                                        <div class="controls">
                                                            <select class="select2 form-control" id="client_id" name="clinic_id" required>
                                                                <option>Select Clinic</option>
                                                                @foreach($clinics as $clinic)
                                                                    <option value="{{$clinic->id}}" 
                                                                    @if(isset($patient->clinic_doctor) && $patient->clinic_doctor->clinic_id == $clinic->id) selected @endif
                                                                    >{{$clinic->name}}</option>
                                                                @endforeach
                                                            </select>
                                                    ...
                                                    <div class="form-group col-xl-6 col-lg-6 col-md-6 col-sm-12 <?php if ($errors->has('clinic_doctor_id')) echo 'error'; ?>">
                                                        ...
                                                        <div class="controls">
                                                            <select class="select2 form-control" id="doctor_id" name="clinic_doctor_id" required >
                                                                @isset($clinic_doctors)
                                                                    @foreach($clinic_doctors as $clinic_doctor)
                                                                        <option value="{{$clinic_doctor->id}}" 
                                                                        @if(isset($patient) && $patient->clinic_doctor_id == $clinic_doctor->id) selected @endif >
                                                                        {{$clinic_doctor->doctor->name}}</option>
                                                                    @endforeach    
                                                                @endisset
                                                    ...
                                                    <div class="form-group col-xl-6 col-lg-6 col-md-6 col-sm-12 <?php if ($errors->has('password_confirmation')) echo 'error'; ?>">
                                                        ...
                                                        <div class="controls">
                                                            <input type="password" name="password_confirmation" class="form-control "
                                                                   data-validation-match-match="password" data-validation-match-message="Must match password">
